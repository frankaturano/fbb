...
document.getElementById('sales-form').addEventListener('submit', function(e){
  ...
  var data = {};
  var fields = document.getElementById('sales-form').querySelectorAll('input, textarea');
  for(var i = 0; i < fields.length; i++){
    var fieldName = fields[i].name;
    var fieldType = fields[i].type;
    var fieldVal = fieldType == "checkbox" ? fields[i].checked : fields[i].value;
    if(fieldName) data[fieldName] = fieldVal;
  }

  var xhr = new XMLHttpRequest();
  xhr.open('POST', 'https://qpb206uwf9.execute-api.us-east-1.amazonaws.com/staging/submit');
  xhr.setRequestHeader('Content-Type', 'application/json');
  xhr.onload = function() {
    if (xhr.status === 200) {
      formDone.innerHTML = '<h1>Five Boroughs Brewing Co. Sales Inquiries</h1>' +
      '<p class="dek">Thanks! This is headed straight to the Five Boroughs sales team, who will be in touch as soon as possible.</p>'
    formDone.classList.add('form-done-complete');
    } else{
      formDone.innerHTML = '<h1>Five Boroughs Brewing Co. Sales Inquiries</h1>' +
      '<p class="dek">Sorry! There was an issue processing your submission. Please email sales@fiveboroughs.com directly. We apologize for the inconvenience.</p>'
    }
  };
